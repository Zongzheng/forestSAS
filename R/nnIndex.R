nnIndex<-function(X,id=1:(X$n),smark=NULL,N=NULL,R=NULL,
  ...
  library(spatstat)
  ppp=rebuild.ppp(X=X,id=id,rm.id=rm.id,add.X=add.X,add.id=add.id)
  ...
  nnid<-nnid(X=ppp,N=N,R=R,id=as.data.frame(ppp)$id,exclude=FALSE)
  if (!is.null(N)){
    nndist <-spatstat::applynbd(X=ppp,N=N,function(dists, ...){sort(dists)},exclude=TRUE)
    if(N==1){
  ...
  if(!is.null(R)){
    ...
    nndist <- spatstat::applynbd(X=ppp,R=R,function(dists, ...){sort(dists)},exclude=TRUE)
